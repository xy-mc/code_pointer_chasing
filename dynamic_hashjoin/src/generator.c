...
random_unique_gen(relation_t *rel) 
{
    ...
    for (i = 0; i < rel->num_tuples; i++) {
        ...
        // printf("%ld\n",rel->tuples[i].key);
        for (int j = 0; j < 32; j++) {
            rel->tuples[i].payload[j] = rand()%(rel->num_tuples/count_gpb);
        }
    }
...
random_unique_gen_thread(void * args) 
{
    ...
    for (i = 0; i < rel->num_tuples; i++) {
        ...
        rel->tuples[i].payload[i] = ridstart + i;
...
random_gen(relation_t *rel, const int64_t maxid) 
{
    ...
    for (i = 0; i < rel->num_tuples; i++) {
        ...
        rel->tuples[i].payload[0] = rand()%(rel->num_tuples/count_gpb);
        for (int j = 1; j < 50; j += 16) {
            rel->tuples[i].payload[j] = rand()%(rel->num_tuples/count_gpb);
        }
    }
